...
feature -- Test routines
	...
	test_1_3_10_1_s9
		...
		do
			assert("Provided local image URL is rendered with proper <img> tags, link is written in the src='***' part. Alt text of the image is properly set.", equal(image1.render (renderer, 0), "<img src='./resources/yoda_1.gif' alt='yoda_1.gif missing'><br>%N"))
			create temp_output_folder.make ("./temp_output")
			assert("temp_output/resources directory was created", temp_output_folder.exists)
			...
			--File 2
			assert("local image file is being copied in temp_output/resources folder", copied_image_1.exists and then equal(original_image_1_content, copied_image_1_content))
			...
			assert("temp_output folder is deleted and created again with new content when new image rendering is called.", copied_image_2.exists and then (equal(original_image_2_content, copied_image_2_content) and not equal(original_image_1_content, copied_image_2_content)))
		end
